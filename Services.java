...
public class Services extends NewAccount {
	...
	public void withdraw(NewAccount ac, Scanner sc) {
		...
		try {
			if(ac.getinitialBalance()<amount) {
				throw new Exception(YELLOW+"Insufficient Balance."+RESET);
			}else {
			...
			System.out.println(e.getMessage());
			System.out.println(GREEN+"Your current balance :"+RED+ac.getinitialBalance()+RESET);
		}
	}
	...
	public int reuse() {
		...
		for (NewAccount ac : nc) {
			if (ac != null) {
				System.out.println(BLUE + "Account Number: " + RESET + ac.getAcNumber() + "   " + BLUE + "user Name: "
						+ RESET + ac.getusername());
				a++;
			}
		}
		return a;
